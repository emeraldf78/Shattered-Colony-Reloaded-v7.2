...
class Workshop: PlayerBuilding {
    ...
    var needsCloseResource: Bool = false  // Waiting for "close building" retrieval
    ...
    override func update(deltaTime: TimeInterval, gameState: GameState) {
        ...
        // If waiting for close resource, check if we can close
        if needsCloseResource {
            // One more collection cycle to "retrieve" the close building resource
    ...
    private func requestCloseResource() {
        // All resources depleted, need one more "retrieval" to close
